...
def getJson(url):
    ...
    r = requests.get('https://api.github.com/repos/' + owner + '/' + repos + '/git/trees/master?recursive=1')
    # if request ok
    if r.ok:
    ...
    # Request Readme Data
    r = requests.get('https://api.github.com/repos/' + owner + '/' + repos + '/readme')
    if r.ok:
    ...
    r = requests.get('https://api.github.com/repos/' + owner + '/' + repos + '/languages')
    if r.ok:
        # LanguageJson -> object
        repoItemLanguage = json.loads(r.text or r.content)

    # Request Commits Data ( up to 100 commits)
    r = requests.get('https://api.github.com/repos/' + owner + '/' + repos + '/commits?per_page=100')
    if r.ok:
        # CommitJson -> Object
        repoItemCommits = json.loads(r.text or r.content)

    # Request Comments Data ( up to 100)
    r = requests.get('https://api.github.com/repos/' + owner + '/' + repos + '/comments?per_page=100')
    if r.ok:
        # CommentJson -> Object
        repoItemsComment = json.loads(r.text or r.context)

    # Request Ussue Data ( Up to 100)
    r = requests.get('https://api.github.com/repos/' + owner + '/' + repos + '/issues?per_page=100')
    if r.ok:
        # IssueJson -> Object
        repoItemIssue = json.loads(r.text or r.context)
    # Request Repo Infos
    r = requests.get('https://api.github.com/repos/' + owner + '/' + repos)
    if r.ok:
        # InfoJson -> Object
...
if __name__ == '__main__':
    getJson('https://github.com/octokit/octokit.net')
